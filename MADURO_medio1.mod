...
%                                                          %
%        Modelo Migración con agentes heterogéneos.        %
%__________________________________________________________%
...
initend= 1*ones(8,1);
% initend = [0.9; 0.9; 3; 3; 0.5; 0.5; 0.5; 0.5];
% options=optimset('Display','iter')
% options.MaxIter = 5000 ;
% options.MaxFunEvals = 20000 ;
% options.StepTolerance = 1e-6;
[xend,fval] = fsolve(@solve_maduro_endval_medio,initend); % Call solver


%initend = 0.5*ones(8,1);

%[xend,fval] = fsolve(@solve_mig5_endval,initend); % Call solver
...
%-----------------------------------------------------------------------------------------------------------------------------------------------------------
   ...
   % Ec. 9 - Foreign - Unskilled
     N_f_u=(1-rhopob)*N_f_u(-1)+rhopob*N_f_u_bar+mig;    
...
%      V_h_s        =(N_h_s*(miubar_s/miu_h_s)^(1/eta))/(1+(((miubar_s*(miu_h_s)^(eta-1))^(1/eta))*(1/x))); 
%      V_h_u        =(N_h_u*(miubar_u/miu_h_u)^(1/eta))/(1+(((miubar_u*(miu_h_u)^(eta-1))^(1/eta))*(1/x))); 
%      V_f_s        =(N_f_s*(miubar_s/miu_f_s)^(1/eta))/(1+(((miubar_s*(miu_f_s)^(eta-1))^(1/eta))*(1/x))); 
%      V_f_u        =(N_f_u*(miubar_u/miu_f_u)^(1/eta))/(1+(((miubar_u*(miu_f_u)^(eta-1))^(1/eta))*(1/x)));  
...
stoch_simul(order=1, irf=40) u u_h_s u_f_s u_h_u u_f_u ;
% simul(periods=1000);
% stoch_simul(order=1,irf=10) w_f_s w_h_s w_f_u w_h_u I K Y u_h_s u_f_s u_h_u u_f_u u; 
